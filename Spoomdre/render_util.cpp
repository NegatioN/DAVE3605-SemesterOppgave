	        ...
	        for(int x = beginx; x <= endx; ++x)
	        {
	            ...
	            if(neighbour != NULL)
	            {
                    ...
                    if(cropYCeiling < nbrYCeil)    {
                    	vLineTexture(renderer, wallTexture, x, yCeiling, nbrYCeil-1, x1, wallHeight, distanceIndex, top, bottom, alpha);
                    }       

	                // If our floor is lower than their floor, render bottom wall
                    if(cropYFloor > nbrYFloor) {
						vLineTexture(renderer, wallTexture, x, nbrYFloor+1, yFloor, x1, wallHeight, distanceIndex, top, bottom, alpha);
                    }
                    ...
                    vLineTexture(renderer, wallTexture, x, yCeiling, yFloor, x1, wallHeight, distanceIndex, top, bottom, alpha);
	            }
	        }
